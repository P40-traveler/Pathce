...
//        Set<String> decoms = decomposer.decompositions.keySet();
    ...
    // Write only triangle entries without the corresponding counts
    public void constructDummy(String queryFile, String catFile) throws Exception {
        Integer patternType = 228;
        ...
        for (Query query : queries) {
            ...
            for (String triangleVList : triangleVLists) {
                String triangleLabelSeq = Util.extractLabelSeq(triangleVList, queryVList, queryLabelSeq);
                catalogue.putIfAbsent(triangleVList, new HashMap<>());
                catalogue.get(triangleVList).put(triangleLabelSeq, 0.);
            }
        }
    ...
    public List<Triple<Integer, String, Double>> getAllCyclicEstimates(Query query, String method, boolean randomSampling) throws Exception {
        ...
        for (String size3entry : decoms) {
            ...
            int subsetSize = size3acyclic.isEmpty() ? 500000 : (500000 / size3acyclic.size());
            est_par = new EstimateParallelCyclic(i, query, catLen, queryVList, startVList, size3decoms, method, EstimateParallelCyclic.Type.ALL, randomSampling, subsetSize);
            ests.add(est_par);
    ...
    public List<Triple<Integer, String, Double>> getAllCyclicEstimatesWithCatLen(Query query, String method, boolean randomSampling, int catLen) throws Exception {
        Pair<String, String> vListAndLabelSeq = Util.topologyToVListAndLabelSeq(query.topology);
        vListAndLabelSeq = Util.sort(vListAndLabelSeq);
        String queryVList = vListAndLabelSeq.key;
        Set<String> alledges = Util.toVListSet(vListAndLabelSeq.key);
        ...
        // acyclic entries
        Set<String> acyclicDecoms = new HashSet<>();
        for (String entry : decoms) {
            Integer[] vList = toVList(entry);
            if (vList.length == 2 * catLen) acyclicDecoms.add(entry);
        }

        // total entries
        Set<String> allDecoms = new HashSet<>();
        allDecoms.addAll(triangles);
        allDecoms.addAll(acyclicDecoms);
        ...
        if (method.contains("onlyExtensionRate")) {
            decoms = acyclicDecoms;
        } else {
            decoms = allDecoms;
        }
        for (String size3entry : decoms) {
            Integer[] startVList = toVList(size3entry);
            int subsetSize = acyclicDecoms.isEmpty() ? 500000 : (500000 / acyclicDecoms.size());
            est_par = new EstimateParallelCyclic(i, query, catLen, queryVList, startVList, allDecoms, method, EstimateParallelCyclic.Type.ALL, randomSampling, subsetSize);
            ests.add(est_par);
            thread = new Thread(est_par);
            threads.add(thread);
            thread.start();
    ...
    public Double[] cyclicEstimation(Query query, String method, boolean randomSampling, boolean allEst, double trueCard, int catLen) throws Exception {

        if (allEst && !randomSampling) {
            List<Triple<Integer, String, Double>> allEstimates = getAllCyclicEstimatesWithCatLen(query, method, randomSampling, catLen);
...
//
    ...
    public static void main(String[] args) throws Exception {
        ...
        } else if (method.contains("dummy")) {
            catalogue.constructDummy(args[1], args[2]);
        }
    }
}
