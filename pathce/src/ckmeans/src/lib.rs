...
//!
//! let input = vec![
//!     1.0, 12.0, 13.0, 14.0, 15.0, 16.0, 2.0, 2.0, 3.0, 5.0, 7.0, 1.0, 2.0, 5.0, 7.0, 1.0, 5.0,
//!     82.0, 1.0, 1.3, 1.1, 78.0,
//! ];
//! let expected = vec![
//!     vec![
...
fn unique_count_sorted<T: CkNum>(input: &[T]) -> usize {
    if input.is_empty() {
        0
    } else {
...
///
/// let input = vec![
///     1.0f64, 12.0, 13.0, 14.0, 15.0, 16.0, 2.0, 2.0, 3.0, 5.0, 7.0, 1.0, 2.0, 5.0, 7.0, 1.0,
...
pub fn ckmeans<T: CkNum>(data: &[T], nclusters: u8) -> Result<Vec<Vec<T>>, CkmeansErr> {
    ...
    let unique_count = unique_count_sorted(&sorted);
...
pub fn ckmeans_lowerbound<T: CkNum>(data: &[T], nclusters: u8) -> Result<Vec<T>, CkmeansErr> {
    if nclusters == 0 {
        return Err(CkmeansErr::TooFewClassesError);
    }
    if nclusters as usize > data.len() {
    ...
    let nvalues = data.len();
    // we'll use this as the maximum number of clusters
    let unique_count = unique_count_sorted(data);
    // if all of the input values are identical, there's one cluster
    // with all of the input in it.
    if unique_count == 1 {
    ...
    // in this way, and this calculation incrementally computes the
    // sum of squares that are later read.
    fill_matrices(data, &mut matrix, &mut backtrack_matrix, nclusters)
        .ok_or(CkmeansErr::ConversionError)?;
    ...
    // once they're generated we can solve for the best clustering groups
    // very quickly.
    let mut clusters: Vec<_> = Vec::with_capacity(nclusters);
    let mut cluster_right = backtrack_matrix[0].len() - 1;
    ...
    for cluster in (0..backtrack_matrix.len()).rev() {
        ...
        clusters.push(data[cluster_left]);
        if cluster > 0 {
            cluster_right = cluster_left - 1;
        }
    }
...
/// resulting in classes ranging “from x to x”. A better approach is to choose the roundest number
/// that separates the lowest point from a class from the highest point
